...
Tcp_Listener *listener_new(const char *addr, uint16_t port)
{
    ...
    if ((listener->tcp_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) ==
        -1)
        return NULL;
...
Tcp_Stream *listener_accept(Tcp_Listener *listener)
{
    ...
    if (listener->tcp_socket == -1 ||
        listener->tcp_len != sizeof(struct sockaddr_in))
        return NULL;
    ...
    if ((stream->stream_socket =
             accept(listener->tcp_socket, addr, &stream->stream_len)) == -1) {
        free(stream);
        return NULL;
    }
...
Tcp_Stream *stream_connect(const char *addr, uint16_t port)
{
    ...
    if ((stream->stream_socket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) ==
        -1) {
        free(stream);
        return NULL;
    }

    if (!inet_pton(AF_INET, addr, &stream->stream_addr)) {
                    ...
                    int flags)
{
    ...
    for (; remain != 0;) {
        ...
        if (pfd.revents & POLLOUT) { // socket fd is ready
            ...
            size_t chunk =
                remain > BUFFER_FRAGMENT_SIZE ? BUFFER_FRAGMENT_SIZE : remain;
